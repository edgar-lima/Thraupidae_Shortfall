...
web<-convert2df("WOS.bib","wos", "bibtex")
scop<- convert2df("scopus.bib","scopus", "bibtex")
```


2. Joining the data from the two databases, removing duplicates and saving the new dataset.
```{r}
dados=  mergeDbSources(scop, web, remove.duplicated = T )
nrow(dados)
write.xlsx(dados, "Dados.xlsx")
```


3. Filtering only the studies that passed through the reading of abstract.
```{r}
screnned<-read_excel("dados.xlsx") 
filtrados<- screnned%>% filter(INCLUDE== "yes")
write.xlsx(filtrados, "Dados2.xlsx")
```
...
```{r}
demo<- read.xlsx("Dados_lacunas.xlsx", sheet = 1)
hist<- read.xlsx("Dados_lacunas.xlsx", sheet = 2)
View(demo)
View(hist)
```
...
```{r}
alldata<- read.xlsx("Dados2.xlsx", sheet = 1)
...
3.1. Calcular o número de artigos por ano separademente para cada lacuna.
```{r}
tdem<- demo%>%select(ID,Year)
tlhist<- hist%>%select(ID,Year)
...
5. Time series of the number of publications standardized by total publications in the area.
```{r}
ggplot(temp, aes(x=Var1, y= publipad))+
  geom_line(size= 1)+
  geom_point(size= 4)+
  scale_x_continuous(limits = c(2003,2020), breaks = seq(2003,2020,1))+
  scale_y_continuous(limits = c(0.0,1.05), breaks = seq(0.0,1.0,0.1))+
  annotate("text", x = temp$Var1, y = temp$posi, 
           label = temp$ano.Freq, size= 5)+
  labs(tag = "a)")+
  xlab("Year") + 
  ylab("Number of standardized studies (%)")+
  theme_bw()+
  theme(panel.grid = element_blank(),
        axis.title = element_text(size = 20, colour = "black"),
        axis.text.x = element_text(size = 16, colour = "black"),
        axis.text.y = element_text(size = 16, colour = "black"),
     panel.border = element_rect(size= 1.2),  plot.tag = element_text(size = 20, face = "bold"))

#ggsave(filename = "tempseries.tiff", width = 34, 
 #       height = 25, units = "cm", dpi = 600)
```

5.1 Figure with two shortfalls.
```{r, fig.height=8}
...
```{r}
dem<- demo%>%distinct(ID, .keep_all = T)%>%
  select(-(Obs:Year),-(ID:Species))
Dem<- ifelse(dem[]!= "NA", 1,0)%>%colSums(na.rm = T)
Dem<- Dem[Dem[]!=0] 
Dem<- data.frame(Dem,names(Dem))
colnames(Dem)<- c("Freq","Param" )
View(Dem)
max(Dem$Freq)
mean(Dem$Freq)
```
...
Obs: Only parameters that presented one or more studies
```{r}
lhist<- hist%>%distinct(ID, .keep_all = T)%>%
  select(-(Obs:Year),-(ID:Species))
Lhist<- ifelse(lhist[]!= "NA", 1,0)%>%colSums(na.rm = T)
Lhist<- Lhist[Lhist[]!=0]
Lhist<- data.frame(Lhist,names(Lhist))
colnames(Lhist)<- c("Freq","Param" )
ParamType<- if_else(
     Lhist$Param =="HomeRangeSize"|Lhist$Param=="HomeRangeSize(SE)"|Lhist$Param=="MixedFlocks"|Lhist$Param=="HabitatUse"|Lhist$Param=="EatingHabit"|Lhist$Param=="Resource(Sp)","Ecological", "Morphological Traits"
    )
Lhist[,3]<- ParamType
Lhist<- cbind.data.frame(Lhist, c(1:nrow(Lhist)))
colnames(Lhist)<- c("Freq", "Param", "G", "ordem")
trat<-c("Ecological" = "darkred","Morphological Traits" = "darkblue") 
View(Lhist)
max(Lhist$Freq)
mean(Lhist$Freq)
```
...
```{r}
ggplot(Lhist, aes(reorder(Param,ordem),Freq, 
                         colour= 
                          G, fill= G))+
  geom_col( width = .40)+
  coord_flip()+
  scale_y_continuous(limits = c(0,15), breaks = seq(0,15,1), )+
  scale_color_manual(values = trat, aesthetics = c("colour", "fill"))+
  xlab("Raunkiæran shortfall parameters") + 
...
ggsave(filename = "Stdemography.tiff", width = 31, 
       height = 25, units = "cm", dpi = 600)
```
